...
async def get_tasks(session: Session = Depends(get_async_session), offset: int = 0, limit: int = 100):
    tasks = await crud.get_tasks(session, offset=offset, limit=limit)
    return tasks    


@router.get("/{task_id}", response_model=schemas.TaskCreate)
async def get_task(task_id: int, session: Session = Depends(get_async_session)):
    task = await crud.get_task_by_id(task_id, session)
    return task 


@router.put("/{task_id}", response_model=schemas.TaskCreate)
async def update_task(task_id: int, session: Session = Depends(get_async_session)):
    task = await crud.get_task_by_id(task_id, session)
...
async def delete_task(task_id: int, session: Session = Depends(get_async_session)):
    task = await crud.get_task_by_id(task_id, session)
